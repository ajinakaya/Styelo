...
import { Upload, Plus, X, Save, Package, Tag, FileText, Camera, Palette, Settings, Info } from 'lucide-react';
...
useEffect(() => {
  const fetchApiData = async () => {
    try {
      ...
      const [categoriesResponse, sectorsResponse, returnPoliciesResponse] = await Promise.all([
        fetch("http://localhost:3001/category/"),
        fetch("http://localhost:3001/sector/"),
        fetch("http://localhost:3001/returnpolicy/")
      ]);

      if (!categoriesResponse.ok || !sectorsResponse.ok || !returnPoliciesResponse.ok) {
  ...
  const addColorVariant = () => {
    setFormData(prev => ({
      ...prev,
      colorOptions: [...prev.colorOptions, { color: '', colorCode: '', furnitureimages: [] }]
    }));
  };
  ...
  const handleSpecificationChange = (type, index, field, value) => {
    ...
    if (type === 'dimensions') {
      ...
      newSpecs.features[index][field] = value;
    }
  ...
  const addSpecificationItem = (type) => {
    ...
    if (type === 'dimensions') {
      ...
      newSpecs.features.push({ label: '', value: '' });
    }
    setFormData(prev => ({ ...prev, specifications: newSpecs }));
  };
  ...
  const removeSpecificationItem = (type, index) => {
    const newSpecs = { ...formData.specifications };
    if (type === 'dimensions') {
      newSpecs.dimensions.additionalDimensions = newSpecs.dimensions.additionalDimensions.filter((_, i) => i !== index);
    } else if (type === 'features') {
      newSpecs.features = newSpecs.features.filter((_, i) => i !== index);
    }
    setFormData(prev => ({ ...prev, specifications: newSpecs }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // JSON fields
    submitData.append('colorOptions', JSON.stringify(formData.colorOptions.map(opt => ({
      color: opt.color,
      colorCode: opt.colorCode
    }))));
    submitData.append('productOverview', JSON.stringify(formData.productOverview.map(item => ({
      label: item.label
    }))));
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          <div className="bg-gradient-to-r from-blue-600 to-purple-600 px-6 py-6">
            <h1 className="text-3xl font-bold text-white">Upload New Furniture</h1>
            <p className="text-blue-100 mt-2">Add your furniture to the catalog with all details</p>
          </div>
              ...
              <div className="space-y-6">
                <div className="bg-gray-50 rounded-lg p-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Furniture Name *</label>
                      <input
                        type="text"
                        value={formData.name}
                ...
                <div className="bg-gray-50 rounded-lg p-6">
                  <div className="flex items-center mb-4">
                    ...
                    <h2 className="text-lg font-semibold text-gray-900">Thumbnail Image</h2>
                  </div>
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-500 transition-colors">
                    ...
                    <label htmlFor="thumbnail-upload" className="cursor-pointer">
                      ...
                      {formData.thumbnail && (
                        <p className="text-sm text-green-600 mt-2">✓ {formData.thumbnail.name}</p>
                      )}
                    </label>
                  </div>
                </div>
                        ...
                        <div className="space-y-3">
                          <div>
                            ...
                            <input
                              type="text"
                              value={item.label}
                              onChange={(e) => handleOverviewChange(index, 'label', e.target.value)}
                              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                          ...
                          <div>
                            ...
                            <div className="border-2 border-dashed border-gray-300 rounded-lg p-3 text-center hover:border-blue-500 transition-colors">
                              <input
                                type="file"
                                accept="image/*"
                                onChange={(e) => handleOverviewIcon(index, e.target.files[0])}
                                className="hidden"
                                id={`icon-upload-${index}`}
                              />
                              <label htmlFor={`icon-upload-${index}`} className="cursor-pointer">
                                <Upload className="h-6 w-6 text-gray-400 mx-auto mb-2" />
                                <p className="text-xs text-gray-600">Upload icon</p>
                                {item.icon && (
                                  <p className="text-xs text-green-600 mt-1">✓ {item.icon.name}</p>
                                )}
                              </label>
                            </div>
                          </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gray-50 rounded-lg p-6">
                  ...
                  <div className="space-y-4 max-h-96 overflow-y-auto">
                    {formData.colorOptions.map((option, index) => (
                      <div key={index} className="border border-gray-200 rounded-lg p-4 bg-white">
                        <div className="flex items-center justify-between mb-3">
                          <h4 className="font-medium text-gray-900 text-sm">Color {index + 1}</h4>
                        ...
                        <div className="space-y-3">
                          ...
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Furniture Images</label>
                            <div className="border-2 border-dashed border-gray-300 rounded-lg p-3 text-center hover:border-blue-500 transition-colors">
                              <input
                                type="file"
                                accept="image/*"
                                multiple
                                onChange={(e) => handleColorVariantImages(index, e.target.files)}
                                className="hidden"
                                id={`images-upload-${index}`}
                              />
                              <label htmlFor={`images-upload-${index}`} className="cursor-pointer">
                                <Upload className="h-6 w-6 text-gray-400 mx-auto mb-2" />
                                <p className="text-xs text-gray-600">Upload images</p>
                                {option.furnitureimages.length > 0 && (
                                  <p className="text-xs text-green-600 mt-1">✓ {option.furnitureimages.length} files</p>
                                )}
                              </label>
                            </div>
                          </div>
                ...
                <div className="bg-gray-50 rounded-lg p-6">
                  ...
                  <div className="space-y-6">
                    {/* Dimensions */}
                    <div>
                      <h3 className="text-md font-medium text-gray-900 mb-3">Dimensions</h3>
                      <div className="space-y-3">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => handleSpecificationChange('dimensions', null, 'overall', e.target.value)}
                              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                          ...
                          <div>
                            ...
                            <input
                              type="text"
                              value={formData.specifications.dimensions.overallProductWeight}
                              onChange={(e) => handleSpecificationChange('dimensions', null, 'overallProductWeight', e.target.value)}
                              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                              placeholder="45 lbs"
                            />
                          </div>
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            <label className="block text-sm font-medium text-gray-700">Additional Dimensions</label>
                            <button
                              type="button"
                          ...
                          <div className="space-y-2">
                            {formData.specifications.dimensions.additionalDimensions.map((dim, index) => (
                              <div key={index} className="flex items-center space-x-2">
                                <input
                                  type="text"
                                  value={dim.label}
                                  onChange={(e) => handleSpecificationChange('dimensions', index, 'label', e.target.value)}
                                ...
                                <input
                                  type="text"
                                  value={dim.value}
                                  onChange={(e) => handleSpecificationChange('dimensions', index, 'value', e.target.value)}
                                  className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                  placeholder="Value"
                                />
                                <button
                                  type="button"
                                  onClick={() => removeSpecificationItem('dimensions', index)}
                                  className="text-red-600 hover:text-red-800"
                                >
                                  <X className="h-4 w-4" />
                                </button>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button
                          type="button"
                          onClick={() => addSpecificationItem('features')}
                          className="text-blue-600 hover:text-blue-800 text-sm"
                        >
                          <Plus className="h-4 w-4" />
                        </button>
                        ...
                        {formData.specifications.features.map((feature, index) => (
                          <div key={index} className="flex items-center space-x-2">
                            <input
                              type="text"
                              value={feature.label}
                              onChange={(e) => handleSpecificationChange('features', index, 'label', e.target.value)}
                              className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            ...
                            <input
                              type="text"
                              value={feature.value}
                              onChange={(e) => handleSpecificationChange('features', index, 'value', e.target.value)}
                              className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                              placeholder="Feature value"
                            />
                            <button
                              type="button"
                              onClick={() => removeSpecificationItem('features', index)}
                              className="text-red-600 hover:text-red-800"
                            >
                              <X className="h-4 w-4" />
                            </button>
